const startSessionController = (dependencies) => async ({ authorizer}) => {
    const { initializeWSBaileysUsecase, getDynamoDBAuthUsecase, postWSApiMessageUsecase, getWsConnectionIdUsecase } = dependencies;
    const {uid} = authorizer;

    const connectionId = await getWsConnectionIdUsecase({params: {userId: uid}});
